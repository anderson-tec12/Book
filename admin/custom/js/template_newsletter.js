...
//btn form-control input-size-90 iframe_modal

/*CSS dos itens do layout*/
var css_titulo = "centro titulo form-control input-size-90";
var css_titulo_left = "form-control input-size-90 input-left";
var css_titulo_tres_colunas = "centro titulo pequeno form-control input-size-90";
...
var css_texto_fixo = "centro texto form-control input-size-90";
var css_texto_fixo_tres_colunas = "centro pequeno form-control input-size-90";
var css_imagem = "btn form-control input-size-90 iframe_modal";
var css_imagem_tres_colunas = "btn form-control input-size-90 iframe_modal";
...
var css_separator = "input-size-90";
var css_icone_item_newsletter = "img-element";
...
function adicionarItem(tipo, texto, idItemNewsletter) {
    ...
    if (typeof ocultaOpcaoPreVisualizar == 'function') {
        ocultaOpcaoPreVisualizar();
    }
...
function addInputText(local, classe, texto, length, tipo, disabled) {
    ...
    inputText.setAttribute('onkeypress', 'actionOnKeyPressInput()');
...
function addTextArea(local, classe, texto, length, tipo, disabled) {
    ...
    textarea.setAttribute('onkeypress', 'actionOnKeyPressInput()');
...
function addSeparator(local, classe) {
    ...
    var inputSeparator = document.createElement("hr");
...
function addIconeItemNewsletter(local, classe, tipo) {
    ...
    icone.setAttribute('src', '../files/templates/newsletter/' + nomeIcone);
...
function addBlocoKappa(local) {
    ...
    addSeparator(local, css_separator);
    addTextArea(local, css_titulo_left, "Você concorda, discorda, tem alguma ressalva a fazer sobre minha iniciativa de busca uma mediação para um acordo?", "subtitulo", false);
    addTextArea(local, css_titulo_left, "Por favor, clique nas opções abaixo para me dizer qual sua avaliação inicial:", "texto_fixo", false);
...
function addBlocoObjetivo(local, values) {
    addSeparator(local, css_separator);
    addTextArea(local, css_titulo_left, values[0] == null ? "Texto 1 do Objetivo" : values[0], length_texto_editavel, "texto_editavel", false);
    addTextArea(local, css_titulo_left, values[1] == null ? "Texto 2 do Objetivo" : values[1], length_texto_editavel, "texto_editavel", false);
    addTextArea(local, css_titulo_left, values[2] == null ? "Texto 3 do Objetivo" : values[2], length_texto_editavel, "texto_editavel", false);
    addInputText(local, css_titulo_left, "Pode por favor me ajudar nesta revisão?", length_texto_fixo, "texto_fixo", true);
    addButton(local, css_titulo_left, "Clique aqui para aceitar este convite e acessar meu caso na plataforma 1Acordo", true);
    addInputText(local, css_titulo_left, "Quer ler a versão atual do meu objetivo ao buscar este acordo? Clique aqui", length_texto_fixo, "texto_fixo", true);
}
...
function actionOnKeyPressInput() {
    if (typeof ocultaOpcaoPreVisualizar == 'function') {
        ocultaOpcaoPreVisualizar();
    }
}
