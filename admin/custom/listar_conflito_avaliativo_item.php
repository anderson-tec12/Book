<?php
if (file_exists(K_DIR . "painel/ferramentas/conflito/bd/conflito_avaliativo_item.php")) {
    require_once K_DIR . "painel/ferramentas/conflito/bd/conflito_avaliativo_item.php";
}

$bdConflitoAvaliativoItem = new ConflitoAvaliativoItem($oConn);
...
<form method="post" name="frm">
    <div class="fieldBox">
        <div class="row">
            <div class="col-xs-12">
                <h1 class="sistem-title">Conflito - Itens Função Avaliativa</h1>
                <div class="form-inline">
                    <h4 class="sistem-subtitle">Filtrar por:</h4>
                    <?
                    $grupos = $bdConflitoAvaliativoItem->getGrupos();
                    ?>
                    ...
                    <table class="table table-hover table-condensed">
                        ...
                        <tbody>
                            ...
                            if ($grupoSelecionado == 'todos') {
                                $lista = $bdConflitoAvaliativoItem->buscaTodos();
                            } else {
                                $lista = $bdConflitoAvaliativoItem->buscaPorGrupo($grupoSelecionado);
                            }
                            ...
                            for ($z = 0; $z <= $fim; $z++) {
                                ...
                                <tr>
                                    <td><?= $item["nome"] ?></td>
                                    <td><?= $bdConflitoAvaliativoItem->getGrupoNomePelaChave($item["grupo"]) ?></td>
                                    <td>
...
<script >
    function load(id)
    {
        ...
        f.action = "index.php?pag=conflito_avaliativo_item&mod=cad&acao=<?= Util::$LOAD ?>&id=" + id + "&tipo=<?= Util::request("tipo") ?>";
        f.submit();
    }
    ...
    function excluir(id)
    {
        ...
        var f = document.forms[0];
        f.action = "delete.php?pag=conflito_avaliativo_item&mod=cad&acao=<?php echo Util::$LOAD ?>&id=" + id + "&tipo=<?= Util::request("tipo") ?>";
        f.submit();
