...
class acao_ticket {
    ...
    public static function getListaTicket($id_usuario, $id_ticket = "", $acao = "", 
            $quem = "", $onde = "", $data = "") {
        ...
        $sql = " select a.*,ac.nome as nome_acao, co.nome as nome_componente, us.nome as quem, us_cad.nome as nome_cad from acao_ticket a 
                ...
                left join usuario us_cad on us_cad.id = ti.id_usuario
                where 
                ( ti.id_usuario = " . $id_usuario . " or a.id_usuario = " . $id_usuario . " ) " ;
...
//caso a ação não exista no banco, ele irá criar uma.
    ...
    public static function geraLogSalvarAnalise($analise, $msg = "") {

        $ferramenta = inc_ferramenta::findFerramentaById($analise['id_ferramenta']);

        $registro = Analise::getDebateByAnalise($analise);

        $nomeTabela = inc_ferramenta::getNomeTabelaDebateByFerramenta($ferramenta['id']);

        if (empty($msg)) {
            $msg = "Salvou uma analise através da ferramenta " . $ferramenta['titulo'] . "(" . $registro['id'] . ")";
        }

        self::gravaLog($msg, "Salvar Analise", $ferramenta['titulo'], $ferramenta['codigo'], $registro['id_ticket'], $registro['id_usuario'], $registro["id"], $nomeTabela, $registro["texto"], "");
    }

    public static function geraLogPublicarAnalise($analise, $msg = "") {
        ...
        if (empty($msg)) {
            $msg = "Publicou uma analise através da ferramenta " . $ferramenta['titulo'] . "(" . $registro['id'] . ")";
        }

        $acao = "Publicar Analise";

        if (empty($analise['id_registro'])) {
            $acao = "Publicar Analise Avulsa";
        }

        self::gravaLog($msg, $acao, $ferramenta['titulo'], $ferramenta['codigo'], $registro['id_ticket'], $registro['id_usuario'], $registro["id"], $nomeTabela, $registro["texto"], "");
    }
...
//$registro["id"]  = Util::numeroBanco( Util::request($prefixo."id") ); 	
        ...
        if (strlen($titulo) > 300){
            $registro["titulo"] = substr($titulo, 0, 300);
        }
        ...
        if ( $url_completa != "")
           $registro["url_completa"] =   $url_completa;
    ...
    public static function buscaComentarioById( $idComentario ) {
        ...
        //$sql = "Select c.*, u.nome_completo from comentario c, usuario u where c.id = " . Util::NVL( $idComentario ,"0") . " and c.id_usuario = u.id";
         $sql = "Select c.*, u.nome_completo from avaliacao_kappa c left join "
                 . " usuario u on u.id = c.id_usuario  "
                 . "where c.id = " . Util::NVL( $idComentario ,"0") . " ";
