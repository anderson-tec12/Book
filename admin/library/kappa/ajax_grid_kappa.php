...
if (file_exists(K_DIR . "inc_ticket_participante.php")) {
...
if ($acao == "salvar_frame") {
    ...
    $objeto_kappa = Util::request("objeto_kappa");
    $url_completa = Util::request("kappa_url_completa");
    
    $completa_titulo_acao = $objeto_kappa != "" ? " em ". $objeto_kappa : "";
    
    acao_ticket::gravaLog("Avaliou com o Kappa".$completa_titulo_acao, "Kappa",
            "Kappa", "Kappa", $id_ticket, $item["id_usuario"], 
            $idAvaliacaoKappa, "avaliacao_kappa", $item["nota"] . " - ".$item["ressalva"] ,
            "","", $url_completa);

    #Rodrigo
    
    if ($item["nota"] == 1 && empty($item["id_avaliacao_kappa_pai"])) {
        TicketParticipante::ressarceCreditoPorKappa1($id_registro, $nome_tabela, $idAvaliacaoKappa);
    }
...
if ($id_registro != "" && $id_ticket != "" && $nome_tabela != "" && ( $acao == "grid" || $acao == "barra" || $acao == "responder")) {
    ...
    $oKappa->mostra_campo_url = false; //Agora há um campo para salvar a url completa da página onde o kappa esta.
    ...
    if ($acao == "barra") {
        $oKappa->mostraBarraComentario($oConn, "images/", $id_pai, $complemento_sql, true, false, false);
    }
    if ($acao == "responder") {
