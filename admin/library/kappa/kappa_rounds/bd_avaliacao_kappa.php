...
require_once (K_DIR . "library/kappa/kappa_rounds/interfaces/interface_gerenciador_alerta_kappa_rounds.php");
...
class BDAvaliacaoKappa {
    ...
    public function contaAvaliacoesPorRegistro($idRegistro, $nomeTabela, $tipoAvaliacao = AvaliacaoKappa::TIPO_AVALIACAO_BASE) {
        ...
        if (!empty($idRegistro) && !empty($nomeTabela)) {
            ...
            #$nomeTabela = $this->tratarNomeTabela($nomeTabela);
    ...
    public function buscaPorRegistro($idRegistro, $nomeTabela, $tipoAvaliacao = null, $nota = null, $idUsuario = null) {
    ...
    public function getNumeroDeKappaPorRegistro($idRegistro, $nomeTabela, $nota = "", $idUsuario = "", $tipoAvaliacao = AvaliacaoKappa::TIPO_AVALIACAO_FINAL) {
        ...
        if ($nomeTabela != null && $idRegistro != null) {
            ...
            return count(connAccess::fetchData($this->conexao, $sql));
        }
    }
    ...
    private function geraAlertaDaAvaliacaoKappa($idAvaliacaoKappa) {
        if (!empty($idAvaliacaoKappa)) {
            ...
            if (!empty($gerenciadorAlerta)) {
                $gerenciadorAlerta->enviarAlertaAoSalvarKappaRounds($avaliacaoKappa);
            }
        }
    }
    ...
    private function tratarNomeTabela($nomeTabela) {

        if (StringHelper::numeroOcorrenciasSubstring($nomeTabela, ":") > 0) {
