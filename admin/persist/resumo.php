...
class Resumo {
	...
	public static function SqlInspecao(){
		...
		$sql = " select i.*, c.nome as nome_cliente, 
				...
				u.nome as nome_responsavel, md.cliente_oculto,
	...
	public static function SqlRelatorio(){
		...
		$sql = " select i.*, c.nome as nome_cliente, 
				concat(e.nome,' / ',e.cidade,' - ',e.estado) as nome_estabelecimento, u.nome as nome_responsavel
				from relatorio i left join cliente c on c.id = i.id_cliente
				left join nutricionista u on u.id = i.id_resp_controllare 
		                left join  estabelecimento e on e.id = i.id_estabelecimento ";
	...
	public static function calculaDados(IDbPersist $oConn, $id, $registro){
		...
		for ( $y = 0 ; $y < count($categorias); $y++){    
			...
			$peso_categoria = connAccess::executeScalar( $oConn," select c.peso
						from categoria c						
						left join peso pe on pe.id =  cast( ifNull(c.peso,'0') as unsigned ) 
						where c.id = " . Util::NVL( $item_c["id_categoria"], "0")."  ");
			...
			for ( $zz = 0; $zz < count($itens); $zz++){
				...
				$peso = connAccess::executeScalar( $oConn," select peso from peso where id in ( select campo1 from 
	...
	public static function exibe_calculoDados(IDbPersist $oConn, $id, $registro){
		
		
		echo("<br><h2>Exibindo cálculos para a inspeção: ". $id . " </h2>");
		...
		for ( $y = 0 ; $y < count($categorias); $y++){    
			...
			//connAccess::executeCommand( $oConn," select peso from peso where id in ( select campo1 from 
			//			cadastro_basico where id = " . Util::NVL( $item_c["peso"], "0").") ");
			
			if ( $peso_categoria == "" || is_null($peso_categoria))
				$peso_categoria = 1;
			
			$catego = connAccess::executeScalar($oConn," select nome from categoria where id = " . $item_c["id_categoria"]);
			
			$str_html .= "<tr><td><b>".$catego."</b><br><div style='padding-left: 20px'>";
			
			for ( $zz = 0; $zz < count($itens); $zz++){
				
				$item = $itens[$zz ];
				
				$nome_item = connAccess::executeScalar($oConn," select descricao from cadastro_basico where id = " . 
						$item["id_item"]);
						
				$str_html .= "<br>-". $nome_item;
				
				if ( $item["nota"] < 0 ){
				
				
					$str_html .= " - <span style='color:red'>Pulou: Nota N/A, não entra no cálculo </span>";
					continue;
				...
				$str_html .= " - <b>Valor da nota:</b> " . $item["nota"]. " , <b>Peso:</b> " . $peso;
			...
			$str_html .="<b>Total de Notas x Peso <b>". $sub_categoria . "  <b> Divide pelo somatório de pesos da categoria</b> " . 
				$divide_categoria . "  ; Valor total desta categoria (". $catego . " ): " .$item_c["nota"] .
				" / <b>Peso desta categoria:</b> "  . $peso_categoria; 
			
			
			
			$str_html .="<br><Br></td></tr>";
		...
		$str_html .="<b>Total de pesos x nota multiplicado por cada peso de categoria <b/>". 
			$total_nota_peso_categoria . " / <b> Somatório de pesos: </b>". $total_pesos_categoria . " / Resultado Total => " . 
			$registro["nota_final"];
	...
	//calcula total sem o peso de categorias.
	public static function calculaDados_old(IDbPersist $oConn, $id, $registro, $imprime = false){
		...
		for ( $y = 0 ; $y < count($categorias); $y++){    
			...
			for ( $zz = 0; $zz < count($itens); $zz++){
				...
				if ( $imprime){
					...
					echo("<br>Peso: ". $peso . ", Nota: " . $item["nota"]. " sub-Total: " . @($peso * $item["nota"]) );
				}
			...
			if ( $imprime){
				...
				echo("<br><b>Total Desta Categoria: ". $sub_categoria . " / Peso deta categoria: " . $divide_categoria. " Avaliaçao desta categoria: " .  @($sub_categoria / $divide_categoria) );
			}
	...
	public static function salvaItens(IDbPersist $oConn,$id_pai,$id_filho, $tabela_pai,
		$tabela_filho, $classificacao, $tipo_pai = "", $tipo_filho= ""){
		//
	...
	public static function filtroClienteNegocio(IDbPersist $oConn, $coluna = "status"){
		...
		if ( $id_perfil == "2" || $id_perfil == "4" || $id_perfil == "5" || $id_perfil == "6"){
			...
			$filtrocompleta = " and ".$alias."id in ( select distinct id_cliente from estabelecimento where id in ( ".$unidades.") ) ";
		}
